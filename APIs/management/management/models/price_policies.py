...
class Price_Policies:
    ...
    def get_price_policy_by_id(self, price_policy_id: int = 1):
        ...
        engine = self.db.engine
        return None if not engine else self.db.get_price_policy_by_id(price_policy_id)
    ...
    def add_price_policy(self, price_policy):
        ...
        engine = self.db.engine
        return None if not engine else self.db.create_price_policy(price_policy)

    def update_price_policy(self, price_policy, price_policy_id):
        """ Update an price_policy given by its id. """
        engine = self.db.engine
        return None if not engine else self.db.update_price_policy(price_policy, price_policy_id)

    def delete_price_policy(self, price_policy_id):
        """ Delete an price_policy given by its id. """
        engine = self.db.engine
        return None if not engine else self.db.delete_price_policy(price_policy_id)